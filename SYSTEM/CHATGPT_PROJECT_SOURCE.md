...
`mehdi_sheet` fournit les caractéristiques et ressources mécaniques actuelles. Pour toute incertitude importante, appeler `validate_check`, puis `roll_check` avec le `headSha` chargé et le prochain `save_id`. Le serveur relit les statistiques canoniques, calcule total, opposition, marge et degré, puis renvoie un reçu chiffré. Reprendre intact le bloc `signed_check` dans l’événement ; `save_turn` vérifie le reçu et reconstruit lui-même les champs mécaniques complets. Montrer seulement `public_display`, jamais `gm_resolution`. Une statistique adverse absente retourne `OPPOSITION_UNRESOLVED` et ne peut pas être inventée.
