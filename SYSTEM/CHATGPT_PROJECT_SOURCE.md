...
`mehdi_sheet` fournit les caractéristiques et ressources mécaniques actuelles. Pour toute incertitude importante, appeler `validate_check`, puis `roll_check` avec le `headSha` chargé et le prochain `save_id`. Le serveur relit les statistiques canoniques, calcule total, opposition, marge et degré, puis renvoie un reçu chiffré compact. Reprendre intact `structuredContent.signed_check` dans l’événement — ou `signed_check` du JSON texte avec le pont historique ; `save_turn` vérifie le reçu et reconstruit lui-même les champs mécaniques complets. Pour une opposition directement perceptible, demander `visibility: public` et montrer `public_display.success_target.dd`, le résultat minimal des dés requis, le total et la marge. Le profil source reste caché. Montrer seulement `public_display`, jamais `gm_resolution`. Une statistique adverse absente retourne `OPPOSITION_UNRESOLVED` et ne peut pas être inventée.
