...
2. Résoudre le tour selon l’interprétation déléguée de Mehdi, mais rendre la main avant tout choix majeur défini par `rules/NARRATION_DARK_FANTASY.md`, sans exposer de secret MJ. Pour toute incertitude mécanique structurée, utiliser `validate_check`, puis `roll_check` avec le `headSha` et le prochain `save_id`, et recopier intact son bloc `signed_check` dans l’événement. `save_turn` vérifie le reçu et reconstruit les champs mécaniques complets. Réserver `roll_dice` aux hasards bruts sans résolution statistique.
