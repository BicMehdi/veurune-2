...
4. Charger `state/MEHDI_PROFILE.yaml` et `state/NARRATIVE_MEMORY.yaml` comme index descriptifs de continuité. Ils ne créent jamais un événement, une émotion ni un choix majeur.
...
Avec Veyrune Cloud Save, préférer `save_turn` en `mode: patch`. Envoyer uniquement les modifications du tour ; le serveur conserve les champs absents, applique les suppressions explicites marquées `null`, reconstruit les six documents complets et exécute ensuite exactement les mêmes validations de continuité, de visibilité et d’append-only que le mode complet. Le mode complet historique est lui aussi fusionné avec l’état courant : une omission ne peut plus effacer silencieusement une donnée.

1. Partir de `state/CURRENT.yaml` validé.
2. Résoudre le tour selon l’interprétation déléguée de Mehdi, mais rendre la main avant tout choix majeur défini par `rules/NARRATION_DARK_FANTASY.md`, sans exposer de secret MJ.
...
6. Mettre à jour `CURRENT.yaml`, `WORLD.yaml`, `HIDDEN.yaml`, `MEHDI_PROFILE.yaml` et `NARRATIVE_MEMORY.yaml` dans la même modification atomique. Toute observation du profil cite une instruction OOC ou des événements joués ; tout résumé de chapitre cite ses événements.
