...
2. Résoudre le tour selon l’interprétation romanesque largement déléguée de Mehdi et poursuivre dialogues, tactiques, combats et réactions cohérentes sans sollicitation intermédiaire. Rendre la main seulement devant un pivot rare réservé par `rules/NARRATION_DARK_FANTASY.md`, sans exposer de secret MJ. Pour toute incertitude mécanique structurée, utiliser `validate_check`, puis `roll_check` avec le `headSha` et le prochain `save_id`, et recopier intact `structuredContent.signed_check` dans l’événement (`signed_check` du JSON texte avec le pont historique). Toute opposition directement perceptible utilise `visibility: public` et son `success_target.dd` est montré au joueur ; le mystère du profil ne suffit pas à cacher ce nombre. Avant le premier test d’un PNJ sans fiche, décider son `npc_class` par fonction narrative ; un PNJ important ou mystérieux peut recevoir un profil secret de conception choisi avant les dés. `save_turn` vérifie le reçu compact, reconstruit les champs mécaniques complets et verrouille classement et profil. Réserver `roll_dice` aux hasards bruts sans résolution statistique.
