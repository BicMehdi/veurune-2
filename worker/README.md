...
`reference/NPC_DESIGN_REGISTRY.json` contient les classements OOC préparés sans créer d’état vivant. Le contact du pont y est `important_mysterious`, avec `mechanical_profile_id: null` : le registre préparé ne présélectionne donc aucune puissance. Si un profil vivant a depuis été signé et sauvegardé dans `HIDDEN`, cet état courant prévaut sans être exposé. Le profil rarissime `NPC-MASTER-CHAMPION` exige toujours trois références et une autorisation préparée explicite lorsqu’il repose sur une conception cachée.
